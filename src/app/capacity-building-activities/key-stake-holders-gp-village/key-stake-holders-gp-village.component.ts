...
@Component({
  selector: 'app-key-stake-holders-gp-village',
  templateUrl: './key-stake-holders-gp-village.component.html',
  styleUrls: ['./key-stake-holders-gp-village.component.scss']
})
export class KeyStakeHoldersGpVillageComponent implements OnInit {
  ...
  removeMeeting(index: number) {
    let allFilesFieldsToDelete: any = {
      ...
      attendance: this.applicationForm.get('rows')['controls'][index].value.attendance,
  ...
  ngOnInit(): void {
    ...
    this.route.url.subscribe((val) => {
      // console.log(val)
      this.formdata.name = `capacity-building-activities/${val.map(v => v.path).join('/')}`
