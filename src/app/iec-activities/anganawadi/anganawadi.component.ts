...
@Component({
  selector: 'app-anganawadi',
  templateUrl: './anganawadi.component.html',
  styleUrls: ['./anganawadi.component.scss']
})
export class AnganawadiComponent implements OnInit {
  ...
  disabledRowLength: number = 0;
  disabledBuildingBoardingLength: number = 0;
  disabledRwhGwrLength: number = 0;
  disabledWallPaingingLength: number = 0;
  totalValues: any = {};
  ...
  addBuildingBoarding() {
    this.formFields = this.applicationForm.get('buildingBoarding') as FormArray;
    const group = this.newBuildingBoarding();
    this.formFields.push(group);
  }

  newBuildingBoarding(): FormGroup {
    return this.formBuilder.group({
      nameNumberAnganvadi: '',
      completedArea: '',
      photo: '',
  ...
  addRwhGwr() {
    this.formFields = this.applicationForm.get('rwhGwr') as FormArray;
    const group = this.newRwhGwr();
    this.formFields.push(group);
  }
  ...
  addWallPainging() {
    this.formFields = this.applicationForm.get('wallPainging') as FormArray;
    const group = this.newWallPainging();
    this.formFields.push(group);
  }

  newWallPainging(): FormGroup {
    return this.formBuilder.group({
      nameOfSchool: '',
      completedArea:'',
      photo: '',
  ...
  ngOnInit(): void {
    this.applicationForm = this.formBuilder.group({
      ...
      buildingBoarding: this.formBuilder.array([
        this.newBuildingBoarding()
      ]),
      rwhGwr: this.formBuilder.array([
        this.newRwhGwr()
      ]),
      wallPainging: this.formBuilder.array([
        this.newWallPainging()
      ]),
      completedDate: ''
    })
  ...
  removeBuildingBranding(index: number) {
    let allFilesFieldsToDelete: any = {
      photo: this.applicationForm.get('buildingBoarding')['controls'][index].value.photo,
  ...
  removeRwhGwr(index: number) {
    let allFilesFieldsToDelete: any = {
      photo: this.applicationForm.get('rwhGwr')['controls'][index].value.photo,
  ...
  removeWallPainging(index: number) {
    let allFilesFieldsToDelete: any = {
      photo: this.applicationForm.get('wallPainging')['controls'][index].value.photo,
  ...
  applicSelected(app: Application) {
    ...
    for (let i = 0; i < app.values.buildingBoarding.length; i++) {
      this.addBuildingBoarding()
    }
    for (let i = 0; i < app.values.rwhGwr.length; i++) {
      this.addRwhGwr()
    }
    for (let i = 0; i < app.values.wallPainging.length; i++) {
      this.addWallPainging()
    }
    this.applicationForm.patchValue(app.values);
    console.log(this.applicationForm)
    this.isFormDisabled = !app.editable;
    this.disabledRowLength = app.values.rows.length;
    console.log(this.applicationForm)
    this.findTotal()
  }
