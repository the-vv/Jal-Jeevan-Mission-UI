...
export class GpwcBoardMeetingComponent implements OnInit {
  ...
  removeMeeting(index: number) {
    let allFilesFieldsToDelete: any = {
      beneficiaryListIndex: this.applicationForm.get('rows')['controls'][index].value.beneficiaryListIndex,
      resolutionIndex: this.applicationForm.get('rows')['controls'][index].value.resolutionIndex
    }
