      ...
      this.rest.uploadFiles(form)
        .subscribe((res) => {
          ...
          if (err.error.status == 'Empty file') {
            ...
            this.snackBar.open('Error uploading file(s), Please try again later', 'Dismiss', { duration: 5000, panelClass: 'bg-danger' })
          }
        })
    }
  }
    ...
    if (update) {
      this.rest.editApplication(app)
        .subscribe(res => {
          ...
          this.submitting = false;
          this.snackBar.open('Error submitting application, Please try again later', 'Dismiss', { duration: 5000, panelClass: 'bg-danger' })
        })
    }
    else {
