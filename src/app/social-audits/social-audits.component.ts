...
@Component({
  ...
  styleUrls: ['./social-audits.component.scss']
})
export class SocialAuditsComponent implements OnInit {
  ...
  removeMeeting(index: number) {
    let allFilesFieldsToDelete: any = {
      reports: this.applicationForm.get('rows')['controls'][index].value.reports
    }
  ...
  ngOnInit(): void {
    ...
    this.route.url.subscribe((val) => {
      // console.log(val)
      this.formdata.name = `social-audits/${val.map(v => v.path).join('/')}`
